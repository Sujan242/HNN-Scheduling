...
i_for_data=0
for ijkl in range(n_iterations*160):
    #pt -processing time | s-size | rt-release time | dd-due date
    pt=ast.literal_eval(data['P'].iloc[i_for_data])
    s=ast.literal_eval(data['S'].iloc[i_for_data])
    rt=ast.literal_eval(data['RT'].iloc[i_for_data])
    dd=ast.literal_eval(data['DD'].iloc[i_for_data])
    ...
    for epochs in range(ep):
        ...
        ciratio=(dd[i]-pt[i])/s[i]
        pw=[w[i]/(rt[i]+pt[i]) for i in range(size)]
...
with open('output_for_FDD_onlytwt.csv', 'w', encoding="ISO-8859-1", newline='') as myfile:
